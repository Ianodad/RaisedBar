...
class Details extends Component {
  ...
  fetchDataFireBase = async(id)=>{
     await firebase.firestore().collection("Cocktail")
.get()
.then(querySnapshot => {
  const data = querySnapshot.docs.map(doc => doc.data());
      this.setState({ data: data });
    });
    console.log(await firebase.firestore().collection('Cocktail').doc(id).get())
    return await firebase.firestore().collection('Cocktail').doc(id).get();
  }
