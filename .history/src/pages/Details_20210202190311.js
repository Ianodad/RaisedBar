...
class Details extends Component {
  ...
  componentDidMount = async () => {
    ...
    console.log(dataCb)
    const { data: dataFb } = await fetchDataFireBase(id)
    console.log(dataFb) 
    this.setState({ cocktailDetail: dataCb ? dataCb.drinks[0] : dataFb  });
  };
  ...
  fetchDataFireBase = async(id)=>{
    return await firebase.firestore().collection('books').doc(id).get();
  }
