...
class Details extends Component {
  ...
  componentDidMount = async () => {
    const { data } = await this.fetchDataCocktailDB()
    console.log(data)
    this.setState({ cocktailDetail: data.drinks[0] });
  };
 
  fetchDataCocktailDB= async()=>{
    return await cocktaildb.getCocktailDetail(this.props.match.params.id);
  }
  fetchDataFireBase = async()=>{
    return  const newCityAdded = await db.collection("cities").add(newCity)
